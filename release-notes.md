...
## 修复
- 修复对话失败时无任何反馈的问题：模型服务异常、agent 进程中断、流意外断开等情况下，界面不再静默无响应，现在会立即弹出具体错误原因
- codex 流式响应意外中断（进程崩溃、订阅断开）现在会正确报错，不再被误当成“成功完成”；流中收到的 error 通知也会被记录并在失败时展示
- 模型返回空内容（无任何文本且未正常结束）时主动报错，不再静默保存一条空回复
- 失败的回复以红色错误卡片持久化在会话记录中，刷新后仍可见，方便追溯；同时保留已生成的部分内容
- 修正会话错误的提示文案（此前所有对话失败都误报为“恢复会话失败”）
...
## 修复
- 修复文件树 related 标签页下只显示当前项目、其它项目文件夹“消失”的问题；其它项目现在照常展示并可展开浏览
- 修复 codex 未登录 ChatGPT 套餐时用量指示器反复报 502/-32600 错误的问题，现在会静默降级为空状态

# MindFS v0.5.3

## 新功能
- 同步上游：markdown 大纲导航、自定义发送快捷键、自定义字体大小

## 优化和修复
- 修复以+/-开头行的 git diff 渲染错误
- 修复重连导致的 ws 状态不稳定
- 修复会话出错后任务仍显示运行中的问题
- 同步上游 e2ee 模式下展开 toolcall 需输入配对码的修复

# MindFS v0.5.2

## 优化和修复
- 修复一键同步后模型列表未分发到 Agent 的问题：同步成功后自动把最新模型静默写入上次手动应用过该供应商的 Agent（如 Pi 的 models.json），不重启会话，Agent 下次启动时生效；同步结果中显示每个 Agent 的应用结果

# MindFS v0.5.1

## 新功能
- 支持模型供应商一键同步：使用已保存的密钥重新拉取全部供应商的模型列表，单个供应商失败时保留旧模型并提示错误
- 支持 API 供应商单模型测试：发送最小测试请求，返回成功/失败、延迟、错误信息与简短响应

## 优化和修复
- 对话 Agent 模型选择器支持输入关键词实时过滤，并按供应商分组展示
- 任务模板与定时任务对话框的模型选择器同步支持关键词搜索

# MindFS v0.5.0

## 新功能
- 同步上游 CodeBuddy Agent 支持、Token 用量与缓存命中率展示
- 支持查看 Agent 内存占用并手动释放会话资源
- 支持收藏和删除快捷提示词，并增加关闭会话自动重命名选项
- 增加开机自启能力，并支持记住文件/看板默认视图

## 优化和修复
- 优化任务看板、会话列表、Tool Call 标题和 Codex 周限额界面
- 断线重连后从最后事件序号继续同步流式回复
- 修复 Codex Provider 与兼容模型保留、Git 提交信息输入和关联文件行号问题
- 修复 E2EE Tool Call 展开、Mermaid 流式渲染和 `.mindfs` Git 忽略问题
- 保留远程 Agent、受保护的 Agent 生命周期接口、Git 远端工作流、自定义主题和服务器部署能力


# MindFS v0.4.16

## 新功能
- 同步上游 DeepSeek Harness、首次使用引导和会话自定义重命名模型
- 支持显示 Codex 周限额、重置时间与可用重置次数
- 支持设置空闲会话资源释放时间，以及将新项目元数据统一存放到 `~/.mindfs`

## 优化和修复
- 优化会话抽屉、任务卡片加载、侧边栏刷新和提示词注入体验
- 修复 Claude Provider 切换与项目路径编码、Codex 工具调用导入及排队消息遮挡问题
- 修复输入法提交、缺失文件返回、Mermaid 竖线标签和关联文件缓存问题
- 移除 Token 加油站的“去加油”入口，保留余额展示与配置生效
- 保留远程 Agent、受保护的 Agent 生命周期接口、Git 远端工作流、自定义主题和服务器部署能力
...
## 新功能
- 同步上游 Agent 配置切换后的重启入口，以及插件加载确认流程
- 同步上游 Codex/Claude 会话导入工具调用、任务 worktree 标识和会话上下文恢复

## 优化和修复
- 优化 Git diff 展示、最终回复复制和回复耗时显示
- 修复删除项目后的残留记录、配置切换残留环境变量和 Windows 终端关闭联动退出问题
- 保留远程 Agent、受保护的 Agent 生命周期接口、Git 远端工作流、自定义主题和服务器部署能力
...
## 新功能
- 同步上游 Grok CLI 支持和中英文国际化
- 同步上游内置任务模板，并支持队列任务立即执行

## 优化和修复
- 同步上游输入历史选择、文件 token 展示、上传进度和模型记忆等改进
- 修复 Codex Originator、Plan 模式、多 Ask User 标题等上游问题
- 保留 fork 的远程 Agent、Git 远端工作流、自定义主题和服务器部署能力


# MindFS v0.4.8

## 优化和修复
- 修复 Linux 目录选择器无法从 `/root` 返回 `/` 的导航问题
- POSIX 路径面包屑增加根目录入口，并使用后端返回的父目录提供返回上级按钮
...
## 新功能
- 添加/切换 API 供应商
- 内置 token station，支持实时余额和一键配置生效

## 优化和修复
- 同步上游的用户消息快速定位、移动端侧边栏、relay 节点判断及浏览器标签页改进
- 兼容旧版 ACP Agent，并修复多轮 `/goal`、Claude 子会话状态及新会话关联文件问题
- 保留远程 Agent、Agent 生命周期管理、模型级思考等级和自定义主题等 fork 功能


# MindFS v0.4.6

## 新功能
- 支持配置远程 MindFS 服务，并在当前实例中使用远程 Agent 与 Shell
- 远程 Agent 保留来源服务器名称、运行能力和会话路由信息

## 优化和修复
- 主输入框仅保留 Agent 切换按钮，模型、Agent 模式、思考等级和 Fast 设置统一收纳到 Agent 菜单
- 恢复手机端输入区可用宽度，避免多个参数按钮并排导致操作按钮被挤出视野
- Agent 菜单按当前模型展示其独立的思考等级和默认值
...
## 优化和修复
- 修复升级后首次打开未创建插件目录的项目时，初始化读取 `.mindfs/plugins` 报路径不存在的问题


# MindFS v0.4.3

## 新功能
- Codex 模型列表支持展示隐藏模型，例如 `gpt-5.6-sol`
- Codex 模型信息透传模型级思考等级和默认思考等级，前端可按模型展示独立选项
- 前端拆分 Agent、模型、Agent 模式、思考等级和 Fast 服务选择器，便于不同任务入口复用

## 优化和修复
- 切换模型时会自动重置为该模型的默认思考等级，避免沿用不匹配的旧配置
- 任务创建、模板和定时任务入口同步适配新的独立选择器
- 补充 Codex 模型映射和 Agent 思考等级推断测试


# MindFS v0.4.2

## 新功能
- Agent 状态新增能力上报，包含 Docker 备份能力和在线更新能力，避免主控持续提示 Agent 未上报 Docker 备份能力
- 新增受保护的 Agent 生命周期接口，主控可直接执行 agents.json 中配置的安装/更新命令
- 前端 Agent 安装和更新入口改为调用主控接口，不再依赖当前项目会话执行 shell 命令
- 前端整体引入 Ant Design 组件体系，统一基础布局、按钮、弹窗、提示和主题入口

## 优化和修复
- Agent 在线更新会逐条执行配置命令，任一命令失败即停止并返回结构化错误与输出
- 更新成功后会终止旧 Agent 进程并触发重新探测，确保版本和能力状态尽快刷新
- Agent 状态 WebSocket 推送同步包含能力字段，避免列表接口和实时刷新状态不一致
- 补充 Agent 能力上报和主控生命周期执行测试，后端测试、前端类型检查和生产构建已通过
# MindFS v0.4.1
